...
for i in range(files):
    print(i)
    data = np.load(f'../output_data/output_weather_{(i+1)*6}h.npy')

    u = np.flip(data[9, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
    v =  np.flip(data[10, :, :], axis=0)[lat_min:lat_max, lon_min:lon_max]
    ws = (u**2+v**2)**0.5
