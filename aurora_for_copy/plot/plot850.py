...
for file_name in file_list:
    if file_name.startswith('aurora_output'):
        files += 1
...
for i in range(files):
    print(i)
    data = nc.Dataset(f'../output_data/aurora_output_{(i)*6+6}hr.nc')
    ...
    u = np.flip(data.variables['upper_variables'][1,-3,:,:], axis=0)[lat_min:lat_max, lon_min:lon_max]
    v =  np.flip(data.variables['upper_variables'][2,-3,:,:], axis=0)[lat_min:lat_max, lon_min:lon_max]
    ws = (u**2+v**2)**0.5
    ...
    plt.title(f'+{i*6+6} hour, 850 mb Wind Speed')
    plt.savefig(f'predict_fine_{i*6+6}.png')
    plt.close()
