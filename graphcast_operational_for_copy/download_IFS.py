...
def download_ifs(IC_time, out_path):
    url = (
        f"https://storage.googleapis.com/ecmwf-open-data/"
        f"{IC_time[:8]}/{IC_time[8:]}z/ifs/0p25/oper/"
        f"{IC_time}0000-0h-oper-fc.grib2"
    )
    print(url)
    r = requests.get(url)
    r.raise_for_status()
...
def read_one_time(IC_time):
    ...
    IC_path = f"input_data/IFS_IC_raw_data_{IC_time}.grib2"
    download_ifs(IC_time, IC_path)
...
def main(IC_time: str)-> None:
    ...
    output_path = "input_data/IFS_initial_condition.nc"
